...
function setUpdateState({ visible = false, version = '', url = '', releaseUrl = '', fileName = '', error = false } = {}) {
    ...
    if (!card || !title || !currentVersion || !latestVersion || !download || !release)
        return;
...
function isStableRelease(info) {
    if (!info || info.draft || info.prerelease)
        return false;
...
async function checkReleaseUpdate({ force = false } = {}) {
    ...
    try {
        ...
        if (!info)
            throw new Error('GitHub returned a non-stable release; stable update channel requires a non-prerelease, non-draft release.');

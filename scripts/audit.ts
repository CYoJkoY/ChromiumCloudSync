...
if (!fs.existsSync(manifestPath))
  throw new Error("Security audit requires a built dist/ directory.");
...
const runtimeCode = runtimeFiles
...
for (const [pattern, label] of forbiddenCode) {
  if (pattern.test(runtimeCode))
    throw new Error(`Forbidden runtime construct detected: ${label}`);
}
...
for (const file of htmlFiles) {
  ...
  if (/<script[^>]+src=["']https?:\/\//i.test(html))
    throw new Error(`Remote script reference in ${file}`);
  if (/<script[^>]*>[^<]+<\/script>/i.test(html))
    throw new Error(`Inline script detected in ${file}`);
}
...
if (manifest.background?.service_worker !== "background.js")
  throw new Error("Manifest service worker must be background.js.");
if (manifest.background?.type !== "module")
  throw new Error("Manifest service worker must remain an ES module.");

console.log(
  `Security audit passed: ${files.length} dist files, ${runtimeFiles.length} JavaScript runtime files.`,
);

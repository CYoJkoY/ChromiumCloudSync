...
execFileSync(tsc, ["-p", path.join(root, "tsconfig.json")], {
  cwd: root,
  stdio: "inherit",
});
...
for (const [sourceFile, outputFile] of runtimeArtifacts) {
  ...
  if (!fs.existsSync(compiled))
    throw new Error(
      `Missing compiled extension file: ${sourceFile} -> ${outputFile}`,
    );
...
for (const [sourceFile, outputFile] of [...pageArtifacts, ...styleArtifacts]) {
  ...
  if (!fs.existsSync(source))
    throw new Error(`Missing extension file: ${sourceFile}`);
...
for (const directory of ["_locales", "icons"]) {
  ...
  if (!fs.existsSync(source))
    throw new Error(`Missing extension directory: ${directory}`);
...
const zipEntries = fs
  .readdirSync(dist)
  .filter((entry) => entry !== path.basename(zip));
execFileSync("zip", ["-qr", zip, ...zipEntries], {
  cwd: dist,
  stdio: "inherit",
});
fs.rmSync(compilerOut, { recursive: true, force: true });

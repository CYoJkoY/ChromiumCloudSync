...
const requiredFiles = [
  'background.js','sync-core.js','legacy-crypto.js','options.js','popup.js','popup-i18n.js',
  'extension-storage.js','extension-storage-layout.js','history.js','guide.js','i18n.js','runtime.js',
  'theme.js','extensions.js'
];
for (const f of requiredFiles) {
  if (!fs.existsSync(path.join(root, f))) throw new Error(`Missing ${f}`);
}
for (const html of ['popup.html','options.html','history.html','guide.html','extensions.html']) {
  if (!fs.existsSync(path.join(root, html))) throw new Error(`Missing ${html}`);
}
...
const filesToScan = [
  'background.js','sync-core.js','popup.js','popup-fixes.js','guide.js','README.md',
  'extensions.js','extensions.html'
];
for (const f of filesToScan) {
  ...
  if (/extensionSettings|applyExtensionSettings|collectExtensionSettings|settingsSyncTimer|applyingRemoteExtensionSettings/.test(text)) {
    throw new Error(`Legacy extension-settings synchronization residue found in ${f}`);
  }
}

const bg = fs.readFileSync(path.join(root, 'background.js'), 'utf8');
if (bg.includes(`[LEGACY_ENCRYPTED_FILE]:{content:null}`)) {
  throw new Error('Invalid Gist PATCH payload: legacy encrypted file must be deleted with a null file value, not null content');
}
if (!bg.includes('if(Object.prototype.hasOwnProperty.call(existingFiles||{},LEGACY_ENCRYPTED_FILE))files[LEGACY_ENCRYPTED_FILE]=null;')) {

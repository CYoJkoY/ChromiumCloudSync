...
assert.equal(
  checksum(withoutVolatileMetadata(merged.snapshot)),
  checksum(withoutVolatileMetadata(repeatedMerge.snapshot)),
);
...
const conflictLocal = snapshot({
  bookmarks: [{ ...base.bookmarks[0], url: "https://local.example" }],
});
const conflictRemote = snapshot({
  bookmarks: [{ ...base.bookmarks[0], url: "https://remote.example" }],
});

  ...
  bookmarks: [
    {
      ...
      title: "Root",
      children: [
        { syncId: "child", title: "Child", url: "https://example.com" },
      ],
    },
  ],
});
...
assert.throws(
  () =>
    migrateCloudState({
      schemaVersion: 99,
      revision: 1,
      snapshot: { extensions: [], windows: [], bookmarks: [] },
    }),
...
assert.throws(
  () =>
    parseAndValidateCloudState({
      ...base,
      snapshot: { ...base.snapshot, extensions: [{ syncId: "bad" }] },
    }),
  (error: unknown) => error instanceof SchemaValidationError,
);

...
if (!fs.existsSync(manifestPath))
  throw new Error(
    "Smoke test requires dist/. Run npm run build:extension first.",
  );
...
const userDataDir = fs.mkdtempSync(
  path.join(os.tmpdir(), "chromium-cloud-sync-smoke-"),
);
let context:
...
try {
  ...
  if (!extensionId)
    throw new Error(
      `Could not resolve the extension ID from service worker URL: ${workerUrl}`,
    );
  ...
  await page
    .locator(".popup-shell")
    .waitFor({ state: "attached", timeout: STAGE_TIMEOUT });
        ...
        new Promise<unknown>((resolve, reject) => {
          const timer = setTimeout(
            () =>
              reject(
                new Error(
                  `runtime.sendMessage timed out after ${messageTimeout} ms.`,
                ),
              ),
            messageTimeout,
          );
  ...
  if (
    ...
    typeof response.capabilities.storage !== "boolean"
  )

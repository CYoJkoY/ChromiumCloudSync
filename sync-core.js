...
export function mergeDeviceStates(devices){
  ...
  const mergeCollection=collection=>{
    ...
    for(const entry of entries){
      ...
      for(const item of arr){const id=objectId(item);if(!id)continue;const key=String(id),candidate={item,updatedAt:entry.updatedAt||''};const prev=map.get(key);if(!prev||String(candidate.updatedAt)>=String(prev.updatedAt))map.set(key,candidate);}
    }
  ...
  const latest=entries.at(-1); merged.device=clone(latest?.snapshot?.device||{});
  const tombstoneMap=new Map();
  for(const entry of entries)for(const t of entry.tombstones||[]){const k=`${t.collection}:${t.syncId}`,prev=tombstoneMap.get(k);if(!prev||String(t.deletedAt)>String(prev.deletedAt))tombstoneMap.set(k,clone(t));}
  const tombstones=[...tombstoneMap.values()]; Object.assign(merged,applyTombstones(merged,tombstones));

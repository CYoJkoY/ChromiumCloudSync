...
function compareVersions(a, b) { const A = versionTuple(a), B = versionTuple(b); for (let i = 0; i < Math.max(A.length, B.length); i++) {
    if ((A[i] || 0) !== (B[i] || 0))
        return (A[i] || 0) - (B[i] || 0);
} return 0; }
function winnerByLatest(local, remote) { const lt = timeOf(local), rt = timeOf(remote); if (lt !== rt)
    return lt > rt ? 'local' : 'remote'; return 'local'; }
export function mergeField(base, local, remote, type, field, conflicts, path) {
    if (stableEqual(local, remote))
        return clone(local);
    if (stableEqual(local, base))
        return clone(remote);
    if (stableEqual(remote, base))
        return clone(local);
    const policy = (MERGE_POLICIES[type] || MERGE_POLICIES.default)[field] || 'conflict';
    if (policy === 'latest') {
        const side = winnerByLatest(local, remote);
        conflicts.push({ type: 'field-auto-resolved', collection: type, syncId: path, field, status: 'resolved', winner: side, strategy: 'latest' });
        return clone(side === 'local' ? local : remote);
    }
    if (policy === 'maxVersion') {
        const side = compareVersions(local, remote) >= 0 ? 'local' : 'remote';
        conflicts.push({ type: 'field-auto-resolved', collection: type, syncId: path, field, status: 'resolved', winner: side, strategy: 'maxVersion' });
        return clone(side === 'local' ? local : remote);
    }
...
export function mergeEntity(base = {}, local = {}, remote = {}, type, conflicts, path) {
    ...
    for (const key of keys) {
        ...
        if (l && r && typeof l === 'object' && typeof r === 'object' && !Array.isArray(l) && !Array.isArray(r)) {
            out[key] = mergeEntity(b || {}, l, r, type, conflicts, `${path}.${key}`);
            continue;
        }
        if (!hasL && !hasR)
            continue;
        if (!hasL) {
            if (hasB && !stableEqual(r, b))
                conflicts.push({ type: 'delete-vs-modify', collection: type, syncId: path, field: key, status: 'unresolved', winner: 'remote' });
            if (hasR)
                out[key] = clone(r);
            continue;
        }
        if (!hasR) {
            if (hasB && !stableEqual(l, b))
                conflicts.push({ type: 'delete-vs-modify', collection: type, syncId: path, field: key, status: 'unresolved', winner: 'local' });
            out[key] = clone(l);
            continue;
        }
...
function mergeArray(base, local, remote, conflicts, path) {
    ...
    if (ids.some(x => !x)) {
        conflicts.push({ type: 'array-conflict', collection: path, syncId: path, status: 'unresolved', winner: 'local', base: clone(B), local: clone(L), remote: clone(R) });
        return clone(L);
    }
    ...
    for (const id of keys) {
        ...
        if (l && !r) {
            if (b && !stableEqual(l, b))
                conflicts.push({ type: 'delete-vs-modify', collection: type, syncId: id, status: 'unresolved', winner: 'local' });
            out.push(clone(l));
            continue;
        }
        if (!l && r) {
            if (b && !stableEqual(r, b))
                conflicts.push({ type: 'delete-vs-modify', collection: type, syncId: id, status: 'unresolved', winner: 'remote' });
            out.push(clone(r));
            continue;
        }
...
export function extractEntities(snapshot) {
    const m = new Map();
    const put = (c, x) => { const id = objectId(x); if (id)
        m.set(`${c}:${id}`, clone(x)); };
    for (const x of snapshot?.extensions || [])
        put('extensions', x);
...
export function mergeTombstones(...sources) { const map = new Map(); for (const list of sources) {
    for (const t of list || []) {
        const k = `${t.collection}:${t.syncId}`, old = map.get(k);
        if (!old || String(t.deletedAt) > String(old.deletedAt))
            map.set(k, clone(t));
    }
} return [...map.values()]; }
export function applyTombstones(snapshot, tombstones = []) {
    const d = new Set((tombstones || []).map(t => `${t.collection}:${t.syncId}`)), out = clone(snapshot || {});
    ...
    for (const w of out.windows)
        w.tabs = (w.tabs || []).filter(t => !d.has(`tabs:${objectId(t)}`));
    out.bookmarks = (out.bookmarks || []).filter(b => !d.has(`bookmarks:${objectId(b)}`));
    return out;
}
export function mergeDeviceStates(devices) {
    ...
    const mergeCollection = collection => {
        ...
        for (const entry of entries) {
            ...
            for (const item of arr) {
                ...
                if (!id)
                    continue;
                const key = String(id), candidate = { item, updatedAt: entry.updatedAt || '' };
                const prev = map.get(key);
                if (!prev || String(candidate.updatedAt) >= String(prev.updatedAt))
    ...
    merged.device = clone(latest?.snapshot?.device || {});
...
export function cleanConflicts(conflicts = []) {
    return (conflicts || []).filter(c => {
        if (!c || c.status === 'resolved' || c.status === 'ignored')
            return false;
        ...
        if (!hasMeaningful && c.type !== 'rollback')
            return false;
        return !!c.collection || !!c.type;
    });
}
export function checksum(value) { let h = 2166136261; const s = JSON.stringify(value); for (let i = 0; i < s.length; i++) {
    h ^= s.charCodeAt(i);

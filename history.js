function setupChoiceSwitch(root, initialValue, onChange) { if (!root)
    return; const bs = [...root.querySelectorAll('.choice-switch-option')]; const set = v => { root.dataset.active = (v === 'en' || v === 'dark') ? 'right' : 'left'; bs.forEach(b => { const active = b.dataset.value === v; b.setAttribute('aria-pressed', String(active)); b.setAttribute('aria-checked', String(active)); }); }; set(initialValue); bs.forEach(b => b.addEventListener('click', async () => { const v = b.dataset.value; try {
    await onChange(v);
    set(v);
}
catch (e) {
...
function installPaginationStyles() {
    if (document.getElementById('historyPaginationStyles'))
        return;
...
function renderHistoryPage() {
    ...
    for (const item of historyItems.slice(startIndex, endIndex)) {
        ...
        btn.textContent = i.t('rollback');
        btn.addEventListener('click', async () => {
            if (!confirm(i.t('rollbackConfirm', { revision: item.sha || '' })))
                return;
            const old = btn.textContent;
            btn.disabled = true;
            btn.textContent = i.t('processing');
            try {
                const x = await request('rollbackHistory', { revision: item.sha });
                statusEl.textContent = i.t('rollbackDone', { revision: x.revision });
                await load();
            }
            catch (error) {
                statusEl.textContent = `${i.t('rollbackFailed')}: ${error.message}`;
            }
            finally {
                btn.disabled = false;
                btn.textContent = old;
...
function renderPagination(start, end, count) {
    ...
    previous.addEventListener('click', () => { if (currentPage > 1) {
        currentPage -= 1;
        renderHistoryPage();
    } });
    ...
    next.addEventListener('click', () => { if (currentPage < totalPages) {
        currentPage += 1;
        renderHistoryPage();
    } });
...
async function load() {
    try {
        ...
        if (!unresolved.length)
            conflictsEl.innerHTML = `<div class="empty">${i.t('noConflicts')}</div>`;
        ...
        statusEl.textContent = `${i.t('historyReadFailed')}: ${error.message}`;
    }
}
...
(async () => {
    ...
    if (langEl) {
        const lv = s.language && s.language !== 'auto' ? s.language : i.currentLanguage();
        setupChoiceSwitch(langEl, lv, async (v) => { await i.setLanguage(v); currentPage = 1; await load(); });
    }
    ...
    if (themeEl) {
        ...
        setupChoiceSwitch(themeEl, tv, async (v) => CCSyncTheme.setTheme(v));
    }
    await load();
})();

const REPO_API =
  "https://api.github.com/repos/CYoJkoY/ChromiumCloudSync/releases/latest";
...
function text(key) {
  const dict = {
    en: {
      ...
      downloadReady:
        "CRX downloaded. Drag the file into chrome://extensions/ to install the update.",
    ...
    "zh-CN": {
      ...
      downloadReady:
        "CRX 已下载。请将文件拖入 chrome://extensions/ 以安装更新。",
...
function normalizeRelease(release) {
  ...
  const asset =
    assets.find((item) => /\.crx$/i.test(String(item.name || ""))) || null;
...
function isStableCachedRelease(cached) {
  if (!cached || cached.prerelease === true || cached.draft === true)
    return false;
...
async function checkReleaseUpdate({ force = false } = {}) {
  ...
  try {
    ...
    if (!info)
      throw new Error(
        "GitHub returned a non-stable release; stable update channel requires a non-prerelease, non-draft release.",
      );
    ...
    if (isStableCachedRelease(cached))
      setUpdateState({ visible: true, ...cached });
...
function initReleaseUpdate() {
  ...
  $u("updateRelease")?.addEventListener("click", (event) => {
    event.preventDefault();
    if (!$u("updateRelease")?.disabled)
      openExternalUrl($u("updateRelease")?.dataset?.url);
  });

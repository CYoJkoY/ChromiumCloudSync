...
export async function readLocal<T extends keyof SyncStorageState>(
  keys: readonly T[],
): Promise<Pick<SyncStorageState, T>>;
...
export async function writeLocal(
  values: Partial<SyncStorageState>,
): Promise<void> {
...
export async function updateLocal<T extends Record<string, unknown>>(
  mutator: (current: SyncStorageState) => T | Promise<T>,
): Promise<T> {

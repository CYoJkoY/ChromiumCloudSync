...
function flattenLegacyBookmarks(
  nodes: unknown,
  parentSyncId: string | null = null,
  out: UnknownRecord[] = [],
): UnknownRecord[] {
  if (!Array.isArray(nodes)) return out;
  for (let i = 0; i < nodes.length; i += 1) {
    const node = isRecord(nodes[i]) ? nodes[i] : {};
    const syncId = isNonEmptyString(node.syncId)
      ? node.syncId
      : `legacy-bookmark-${crypto.randomUUID()}`;
...
function normalizeSnapshotShape(input: unknown): Snapshot {
  ...
  if (
    ...
    value.bookmarks.some(
      (entry) => isRecord(entry) && Array.isArray(entry.children),
    )
  )
    value.bookmarks = flattenLegacyBookmarks(value.bookmarks);
  delete value.device;
...
function migrateLegacyState(
  ...
  if (fromVersion <= 9 && isRecord(current.devices)) {
    const merged = mergeDeviceStates(
      current.devices as Record<string, CloudState | null | undefined>,
    );
    current = {
      schemaVersion: SCHEMA_VERSION,
      revision: Math.max(
        0,
        ...Object.values(current.devices)
          .filter(isRecord)
    ...
    current = {
      ...
      tombstones: Array.isArray(current.tombstones)
        ? (current.tombstones as Tombstone[])
        : [],
      conflicts: Array.isArray(current.conflicts)
        ? (current.conflicts as ConflictRecord[])
        : [],
    };
  }
...
export function migrateCloudState(input: unknown): CloudState {
  if (!isRecord(input))
    throw new SchemaValidationError("Cloud state must be a JSON object.");
  const embeddedSnapshot = isRecord(input.snapshot) ? input.snapshot : input;
  const rawVersion = Number(
    input.schemaVersion ?? embeddedSnapshot.schemaVersion ?? 0,
  );
  const fromVersion =
    Number.isFinite(rawVersion) && rawVersion > 0 ? rawVersion : SCHEMA_VERSION;
  if (fromVersion > SCHEMA_VERSION)
    throw new SchemaValidationError(
      `Unsupported future schema ${fromVersion}; current schema is ${SCHEMA_VERSION}.`,
    );
  if (
    !SUPPORTED_SCHEMA_VERSIONS.includes(fromVersion as SupportedSchemaVersion)
  )
    throw new SchemaValidationError(
      `Unsupported schema ${fromVersion}; supported versions are ${SUPPORTED_SCHEMA_VERSIONS.join(", ")}.`,
    );
  if (fromVersion === SCHEMA_VERSION) {
    return validateCloudState({
      ...
      tombstones: Array.isArray(input.tombstones)
        ? (input.tombstones as Tombstone[])
        : [],
      conflicts: Array.isArray(input.conflicts)
        ? (input.conflicts as ConflictRecord[])
        : [],
    });
  }
...
function validateExtension(
  value: unknown,
  index: number,
): asserts value is ExtensionRecord {
  if (!isRecord(value))
    throw new SchemaValidationError(
      `snapshot.extensions[${index}] must be an object.`,
    );
  for (const field of ["syncId", "id", "name", "version"] as const)
    if (!isNonEmptyString(value[field]))
      throw new SchemaValidationError(
        `snapshot.extensions[${index}].${field} must be a non-empty string.`,
      );
  if (typeof value.enabled !== "boolean")
    throw new SchemaValidationError(
      `snapshot.extensions[${index}].enabled must be boolean.`,
    );
  for (const field of [
    "description",
    "homepageUrl",
    ...
    if (value[field] !== undefined && typeof value[field] !== "string")
      throw new SchemaValidationError(
        `snapshot.extensions[${index}].${field} must be a string when present.`,
      );
}

function validateTabGroup(
  value: unknown,
  path: string,
): asserts value is TabGroupRecord {
  if (!isRecord(value))
    throw new SchemaValidationError(`${path} must be an object.`);
  if (!isNonEmptyString(value.syncId))
    throw new SchemaValidationError(
      `${path}.syncId must be a non-empty string.`,
    );
  if (
    value.localId !== undefined &&
    (!isFiniteNumber(value.localId) || value.localId < 0)
  )
    throw new SchemaValidationError(
      `${path}.localId must be a non-negative finite number when present.`,
    );
  if (
    value.windowId !== undefined &&
    (!isFiniteNumber(value.windowId) || value.windowId < 0)
  )
    throw new SchemaValidationError(
      `${path}.windowId must be a non-negative finite number when present.`,
    );
  if (typeof value.title !== "string")
    throw new SchemaValidationError(`${path}.title must be a string.`);
  if (!isNonEmptyString(value.color))
    throw new SchemaValidationError(
      `${path}.color must be a non-empty string.`,
    );
...
function validateTab(value: unknown, path: string): asserts value is TabRecord {
  ...
  if (!isNonEmptyString(value.url) || !/^https?:\/\//i.test(value.url))
    throw new SchemaValidationError(
      `${path}.url must be a non-empty http(s) URL.`,
    );
  if (typeof value.title !== "string")
    throw new SchemaValidationError(`${path}.title must be a string.`);
  if (typeof value.pinned !== "boolean" || typeof value.active !== "boolean")
    throw new SchemaValidationError(
      `${path}.pinned and ${path}.active must be boolean.`,
    );
  if (
    ...
    throw new SchemaValidationError(
      `${path}.index must be a non-negative finite number when present.`,
    );
  if (value.group !== undefined && value.group !== null)
    validateTabGroup(value.group, `${path}.group`);
}

function validateWindow(
  value: unknown,
  index: number,
): asserts value is WindowRecord {
  ...
  if (!isNonEmptyString(value.state))
    throw new SchemaValidationError(
      `${path}.state must be a non-empty string.`,
    );
  if (typeof value.focused !== "boolean")
    throw new SchemaValidationError(`${path}.focused must be boolean.`);
  if (!Array.isArray(value.tabs))
    throw new SchemaValidationError(`${path}.tabs must be an array.`);
  value.tabs.forEach((tab, tabIndex) =>
    validateTab(tab, `${path}.tabs[${tabIndex}]`),
  );
}
...
function validateBookmark(
  value: unknown,
  index: number,
): asserts value is BookmarkRecord {
  ...
  if (value.parentSyncId !== null && !isNonEmptyString(value.parentSyncId))
    throw new SchemaValidationError(
      `${path}.parentSyncId must be null or a non-empty string.`,
    );
  if (!isFiniteNumber(value.index) || value.index < 0)
    throw new SchemaValidationError(
      `${path}.index must be a non-negative finite number.`,
    );
  if (typeof value.title !== "string")
    throw new SchemaValidationError(`${path}.title must be a string.`);
  if (
    value.url !== undefined &&
    (!isNonEmptyString(value.url) || !/^https?:\/\//i.test(value.url))
  )
    throw new SchemaValidationError(
      `${path}.url must be a non-empty http(s) URL when present.`,
    );
}
...
function validateTombstone(
  value: unknown,
  index: number,
): asserts value is Tombstone {
  const path = `tombstones[${index}]`;
  if (!isRecord(value))
    throw new SchemaValidationError(`${path} must be an object.`);
  if (!isNonEmptyString(value.collection) || !isNonEmptyString(value.syncId))
    throw new SchemaValidationError(
      `${path}.collection and ${path}.syncId must be non-empty strings.`,
    );
  if (!isNonEmptyString(value.deletedAt))
    throw new SchemaValidationError(
      `${path}.deletedAt must be a non-empty string.`,
    );
  if (!isFiniteNumber(value.revision) || value.revision < 0)
    throw new SchemaValidationError(
      `${path}.revision must be a non-negative finite number.`,
    );
}
...
function validateConflict(
  value: unknown,
  index: number,
): asserts value is ConflictRecord {
  const path = `conflicts[${index}]`;
  if (!isRecord(value))
    throw new SchemaValidationError(`${path} must be an object.`);
  if (!isNonEmptyString(value.type) || !isNonEmptyString(value.status))
    throw new SchemaValidationError(
      `${path}.type and ${path}.status must be non-empty strings.`,
    );
  for (const field of [
    "collection",
    "syncId",
...
export function validateCloudState(state: CloudState): CloudState {
  if (!isRecord(state))
    throw new SchemaValidationError("Cloud state must be an object.");
  if (state.schemaVersion !== SCHEMA_VERSION)
    throw new SchemaValidationError(
      `Expected schema ${SCHEMA_VERSION}, received ${state.schemaVersion}.`,
    );
  if (!isFiniteNumber(state.revision) || state.revision < 0)
    throw new SchemaValidationError(
      "Revision must be a non-negative finite number.",
    );
  if (typeof state.updatedAt !== "string" || !state.updatedAt)
    throw new SchemaValidationError(
      "updatedAt must be a non-empty timestamp string.",
    );
  if (!isRecord(state.snapshot))
    throw new SchemaValidationError("snapshot must be an object.");
  if (!Array.isArray(state.snapshot.extensions))
    throw new SchemaValidationError("snapshot.extensions must be an array.");
  if (!Array.isArray(state.snapshot.windows))
  ...
  state.snapshot.bookmarks.forEach((item, index) =>
    validateBookmark(item, index),
  );
  if (!Array.isArray(state.tombstones))
    throw new SchemaValidationError("tombstones must be an array.");
  if (!Array.isArray(state.conflicts))
    throw new SchemaValidationError("conflicts must be an array.");
  ...
  for (const collection of [
    ...
    for (const item of state.snapshot[collection]) {
      ...
      if (seen.has(key))
        throw new SchemaValidationError(`Duplicate syncId detected: ${key}.`);
      seen.add(key);
    }
  return state;
}

...
export async function importAesKey(raw, extractable = false) {
  return crypto.subtle.importKey("raw", raw, { name: AES }, extractable, [
    "encrypt",
    "decrypt",
  ]);
}
...
export async function deriveKek(recoveryCode, saltB64) {
  if (!recoveryCode || recoveryCode.length < 12)
    throw new Error("恢复码至少需要 12 个字符");
  return derivePasswordKek(recoveryCode, saltB64, PBKDF2_ITERATIONS);
}
export async function deriveConvenienceKey(
  githubToken,
  saltB64,
  iterations = CONVENIENCE_PBKDF2_ITERATIONS,
) {
...
async function encryptBytes(bytes, key) {
  ...
  const cipher = new Uint8Array(
    await crypto.subtle.encrypt({ name: AES, iv, tagLength: 128 }, key, bytes),
  );
...
export async function decryptBytes(payload, key) {
  return new Uint8Array(
    await crypto.subtle.decrypt(
      { name: AES, iv: base64ToBytes(payload.iv), tagLength: 128 },
      key,
...
export async function wrapMasterKey(masterRaw, secret, mode = "convenience") {
  ...
  const iterations =
    mode === "recovery" ? PBKDF2_ITERATIONS : CONVENIENCE_PBKDF2_ITERATIONS;
  const kek =
    mode === "recovery"
      ? await deriveKek(secret, salt)
      : await deriveConvenienceKey(secret, salt, iterations);
  return {
    salt,
    iterations,
...
export async function unwrapMasterWithSecret(
  secret,
  wrapper,
  mode = wrapper?.type,
) {
  if (!wrapper?.salt || !wrapper?.wrappedMasterKey)
    throw new Error("云端缺少密钥包装材料");
  ...
  const kek =
    mode === "recovery"
      ? await deriveKek(secret, wrapper.salt)
      : await deriveConvenienceKey(secret, wrapper.salt, iterations);
  try {
...
export async function decryptJson(text, key) {
  ...
  if (payload?.v !== 7 || payload?.alg !== AES)

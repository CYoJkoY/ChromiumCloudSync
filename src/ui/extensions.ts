...
const dict = {
  en: {
    ...
    extensionRecoverySubtitle:
      "Find missing extensions and restore the browser environment.",
    refresh: "Refresh",
    loading: "Loading…",
    missingExtensionsTitle: "Missing extensions",
    missingExtensionsDesc:
      "These extensions exist in the cloud snapshot but are not installed on this browser.",
    recoveryPolicyTitle: "Recovery policy",
    recoveryPolicyBody:
      "Chromium Cloud Sync records extension metadata and identifies missing extensions. Store installation stays a user action. Third-party CRX / ZIP packages are handled separately by the configured package-backup backend.",
    recoveryPolicyNote:
      "The recovery center never writes into third-party extension storage and never synchronizes third-party extension settings.",
    packageRecoveryTitle: "Package recovery",
    packageRecoveryBody:
      "For extensions that are not available from a browser store, use the package-backup controls in Settings to download a stored CRX / ZIP file.",
    openSettings: "Open Settings",
    noMissing:
      "No missing extensions. This browser matches the cloud extension inventory.",
  ...
  "zh-CN": {
    ...
    recoveryPolicyBody:
      "Chromium Cloud Sync 只记录扩展元数据并负责发现缺失扩展。商店安装始终由用户确认；第三方 CRX / ZIP 文件则由单独配置的扩展包备份后端负责。",
    recoveryPolicyNote:
      "恢复中心不会写入第三方扩展的内部存储，也不会同步第三方扩展设置。",
    packageRecoveryTitle: "扩展包恢复",
    packageRecoveryBody:
      "对于无法从浏览器商店获取的扩展，可以在“设置”中的第三方扩展文件存储区域下载已经备份的 CRX / ZIP 文件。",
    openSettings: "打开设置",
...
function storeAction(ext) {
  ...
  if (source === "edge" && ext.store.edgeUrl)
    return { url: ext.store.edgeUrl, label: t("openEdge") };
  if (source === "chrome" && ext.store.chromeUrl)
    return { url: ext.store.chromeUrl, label: t("openChrome") };
  if (ext.homepageUrl && /^https?:\/\//i.test(ext.homepageUrl))
    return { url: ext.homepageUrl, label: t("openHomepage") };
  return null;
}
...
function render(data) {
  ...
  if (data.remoteUnavailable) {
    ...
    summary.textContent =
      `${t("remoteUnavailable")} ${data.errorMessage || ""}`.trim();
    return;
  }
...
function applyText() {
  ...
  document.querySelectorAll("[data-i18n]").forEach((node) => {
    ...
    if (dict[currentLanguage]?.[key])
      node.textContent = dict[currentLanguage][key];
  });
...
async function syncLanguageFromStorage() {
  ...
  const storedLanguage =
    saved.language && saved.language !== "auto" ? saved.language : "";
  currentLanguage = normalizeLanguage(
    sharedLanguage || storedLanguage || navigator.language || "en",
  );
  document.documentElement.lang = currentLanguage;
  applyText();
}
...
(async () => {
  try {
    ...
    setupChoiceSwitch(languageRoot, currentLanguage, async (value) => {
      ...
      if (
        window.CCSyncI18n?.setLanguage &&
        window.CCSyncI18n.currentLanguage?.() !== currentLanguage
      ) {
        try {
    ...
    if (window.CCSyncTheme) {
      ...
      setupChoiceSwitch(
        ...
        async (value) => CCSyncTheme.setTheme(value),
      );
    }

...
function paginationText(key, vars = {}) {
  ...
  return String(dict[lang][key] || dict.en[key] || key).replace(
    /\{(\w+)\}/g,
    (_m, k) => vars[k] ?? "",
  );
}
...
function renderPagination(start, end, count) {
  ...
  state.textContent = paginationText("page", {
    current: currentPage,
    total: totalPages,
  });
...
async function load() {
  try {
    ...
    currentPage = Math.min(
      currentPage,
      Math.max(1, Math.ceil(historyItems.length / PAGE_SIZE)),
    );
    renderHistoryPage();

    conflictsEl.replaceChildren();
    const unresolved = (r.state?.conflicts || []).filter(
    ...
    statusEl.textContent = [
      i.t("gistStatus", { gist: r.gistId || i.t("notConfigured") }),
      i.t("revisionStatus", { revision: r.revision ?? 0 }),
      i.t("historyCount", { count: historyItems.length }),
      i.t("unresolvedCount", { count: unresolved.length }),
    ].join("\n");
  } catch (error) {
...
(async () => {
  ...
  const s = await new Promise((resolve, reject) =>
    chrome.storage.local.get(["language"], (v) =>
  ...
  if (langEl) {
    ...
    setupChoiceSwitch(langEl, lv, async (v) => {

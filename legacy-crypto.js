...
export async function derivePasswordKek(secret, saltB64, iterations = PBKDF2_ITERATIONS) {
    if (!secret)
        throw new Error('密钥材料不能为空');
    const base = await crypto.subtle.importKey('raw', te.encode(secret), 'PBKDF2', false, ['deriveKey']);
    return crypto.subtle.deriveKey({ name: 'PBKDF2', salt: base64ToBytes(saltB64), iterations, hash: 'SHA-256' }, base, { name: AES, length: 256 }, false, ['encrypt', 'decrypt']);
}

(function () {
    ...
    function apply() {
        ...
        const map = [['updateDownload', 'download'], ['updateRelease', 'release'], ['checkUpdates', 'check']];
        for (const [id, key] of map) {
            const el = $(id);
            if (el && !el.dataset.updateManaged)
                el.textContent = d[key];
        }
        const l = $('language'), th = $('theme');
        if (l)
            l.setAttribute('aria-label', d.language);
        if (th)
            th.setAttribute('aria-label', d.theme);

const DICT = {
    ...
    'zh-CN': {
        ...
        packageTitle: '第三方扩展包备份', packageBody: 'CRX / ZIP 备份与浏览器状态同步完全分离。打开“设置 → 第三方扩展”，选择“GitHub 私有仓库”或 WebDAV，填写对应凭据，测试并保存后，再选择需要备份的已安装扩展。对于已选择的扩展，点击“备份 CRX / ZIP”并手动选择对应文件上传。扩展包索引和选择结果保存在所选后端；凭据只保存在当前浏览器本地。扩展安装仍然需要手动完成。',
        historyTitle: '历史与回滚', historyBody: 'GitHub Gist 保存同步状态的 Revision 历史。插件还会在本地维护最多 30 条历史索引记录。“历史”页面可以查看版本，并将指定版本回滚成新的当前 Revision。',
        settingsTitle: '设置页面', settingsBody: '设置页面将同步配置与第三方扩展包存储分开。主 GitHub Token 用于 Gist 同步；第三方扩展包存储使用独立凭据：GitHub 私有仓库需要单独的 GitHub Token，并且只需给目标仓库 Contents 写权限；WebDAV 则使用独立的地址和可选的用户名/密码。这些扩展包凭据只保存在当前浏览器本地，不会进入浏览器状态同步快照。',
        privacyTitle: '隐私与安全', privacyBody: '插件创建的同步 Gist 默认是私有的。当前同步载荷以普通 JSON 保存在 Gist 中，并不是端到端加密。应将能够访问这个私有 Gist 视为能够访问同步的浏览器数据。扩展包备份建议使用私有 GitHub 仓库或可信的 WebDAV 服务，并坚持最小权限原则。',
        troubleTitle: '故障排查', troubleBody: '同步失败时，先打开 Popup 查看详细状态信息，然后在设置中检查 GitHub Token 和 Gist 绑定。扩展包备份失败时，检查所选后端、凭据、仓库或 WebDAV 路径，以及已保存的扩展选择。浏览器不能自动向另一个扩展暴露其已安装 CRX 的原始字节，因此首次备份扩展包需要手动选择文件。',
        scopeTitle: '重要的功能边界', scopeBody: 'Chromium Cloud Sync 不同步第三方扩展设置。这是有意的设计：扩展设置属于各自扩展的私有存储，格式差异很大，Chromium 扩展也无法安全地通用处理其他扩展的内部数据。因此项目只同步扩展清单，并提供独立的扩展包备份与恢复路径。'
    }
};
function detect() { return [navigator.language, ...(navigator.languages || [])].some(x => /^zh(?:[-_]|$)/i.test(x)) ? 'zh-CN' : 'en'; }
function apply() { const s = localStorage.getItem('ccsync-guide-language') || 'auto', l = s === 'auto' ? detect() : (s === 'zh-CN' ? 'zh-CN' : 'en'), d = DICT[l] || DICT.en; document.documentElement.lang = l; document.querySelectorAll('[data-guide]').forEach(e => { const value = d[e.dataset.guide]; if (value)
    e.textContent = value; }); }
function setupChoiceSwitch(root, initialValue, onChange) { if (!root)
    return; const bs = [...root.querySelectorAll('.choice-switch-option')], set = v => { root.dataset.active = (v === 'en' || v === 'dark') ? 'right' : 'left'; bs.forEach(b => b.setAttribute('aria-pressed', String(b.dataset.value === v))); }; set(initialValue); bs.forEach(b => b.addEventListener('click', async () => { const v = b.dataset.value; await onChange(v); set(v); })); }
(async () => { const lang = document.getElementById('language'); const s = await chrome.storage.local.get(['language']); if (lang) {
    const lv = s.language && s.language !== 'auto' ? s.language : detect();
    setupChoiceSwitch(lang, lv, async (v) => { await chrome.storage.local.set({ language: v }); localStorage.setItem('ccsync-guide-language', v); apply(); });
} const theme = document.getElementById('theme'); if (theme && window.CCSyncTheme) {

...
bindAction('checkExtensions', async (_event, button) => withButton(button, async () => {
    ...
    for (const ext of r.missing) {
        ...
        a.href = ext.store?.source === 'edge' ? ext.store.edgeUrl : ext.store.chromeUrl;
        a.target = '_blank';
        a.rel = 'noreferrer';
...
function setupChoiceSwitch(root, initialValue, onChange) { if (!root)
    return; const buttons = [...root.querySelectorAll('.choice-switch-option')]; const set = value => { root.dataset.active = (value === 'en' || value === 'dark') ? 'right' : 'left'; for (const b of buttons)
    b.setAttribute('aria-pressed', String(b.dataset.value === value)); }; set(initialValue); for (const b of buttons)
    b.addEventListener('click', async () => { const value = b.dataset.value; try {
...
(async () => { try {
    ...
    if (ui.lang) {
        ...
        setupChoiceSwitch(ui.lang, lv, async (v) => { await i.setLanguage(v); await refresh(); });
    }

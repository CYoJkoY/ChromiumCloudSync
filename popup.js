...
bindAction('checkExtensions', async (_event, button) => withButton(button, async () => {
  ...
  for (const ext of r.missing) {
    ...
    const title = document.createElement('div'); title.className = 'history-title'; title.textContent = ext.name || ext.id;
    const meta = document.createElement('div'); meta.className = 'history-meta'; meta.textContent = ext.id;
    main.append(title, meta);
    const a = document.createElement('a');
    a.href = ext.store?.source === 'edge' ? ext.store.edgeUrl : ext.store.chromeUrl;
    a.target = '_blank'; a.rel = 'noreferrer'; a.className = 'secondary'; a.textContent = i.t('openInstall');
...
function setupChoiceSwitch(root, initialValue, onChange) {
  ...
  for (const b of buttons) b.addEventListener('click', async () => {
    ...
    try { await onChange(value); set(value); }

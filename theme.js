(function () {
    ...
    function normalizeControls() {
        ...
        if (dark) {
            ...
            dark.setAttribute('aria-label', label);
            dark.setAttribute('title', label);
        }
    }
    function animate() {
        const root = document.getElementById('theme');
    ...
    media?.addEventListener?.('change', async () => { if ((await getTheme()) === 'system')
        applyTheme('system'); });
    window.CCSyncTheme = { getTheme, initTheme, setTheme, applyTheme, normalizeControls };
    if (document.readyState === 'loading')
        document.addEventListener('DOMContentLoaded', () => { injectStyles(); normalizeControls(); void initTheme(); });
    else {
        injectStyles();
        normalizeControls();
        void initTheme();
